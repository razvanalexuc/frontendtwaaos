...
PROJECT_KEY = "backendtwaaos"  # Cheia proiectului backend
...
if response.status_code != 200:
    print("ERROR: Eroare la conectarea cu SonarQube:", response.text)
    exit()
...
if not issues:
    print("WARNING: Nu s-au gasit probleme in proiect.")
    exit()
...
output_filename = f"sonarqube_backend_report_{timestamp}.txt"
output_path = os.path.join(raports_dir, output_filename)

# Creare raport text
with open(output_path, "w", encoding="utf-8") as f:
    f.write(f"Raport SonarQube Backend - {PROJECT_KEY}\n")
    f.write(f"Data generarii: {data_curenta}\n")
    f.write("=" * 80 + "\n\n")
    
    if not issues:
        f.write("Nu s-au gasit probleme in proiect.\n")
    else:
        f.write(f"Total probleme gasite: {len(issues)}\n\n")
